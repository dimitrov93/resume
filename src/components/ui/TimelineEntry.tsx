    ...
    >
      ...
      <div className="absolute left-0 top-3 -translate-x-1/2">
        <div className="w-3 h-3 rounded-full bg-accent/20 flex items-center justify-center">
          <div className="w-2 h-2 rounded-full bg-accent shadow-[0_0_8px_rgba(255,219,110,0.4)]" />
        </div>
      </div>

      <div className="group bg-card border border-border-gold rounded-2xl p-5 transition-all duration-300 hover:border-accent/30 hover:shadow-[0_4px_20px_rgba(255,219,110,0.06)]">
        <span className="inline-block text-[11px] px-3 py-1 rounded-full bg-gold-glow text-accent font-medium mb-3 tracking-wide">
          {period}
        </span>
        <h3 className="font-semibold text-white text-[15px] mb-2 group-hover:text-accent transition-colors duration-300">
          {title}
        </h3>
              ...
              {roles.map((r, i) => (
                <div key={r.role} className="flex items-center gap-3">
                  <div className="flex items-center gap-1.5 shrink-0">
                    <div className={`w-1.5 h-1.5 rounded-full ${i === 0 ? 'bg-accent shadow-[0_0_6px_rgba(255,219,110,0.4)]' : 'bg-white/20'}`} />
                    {i < roles.length - 1 && (
                      <div className="absolute ml-[2.5px] mt-6 w-px h-3 bg-white/10" />
                    )}
                  </div>
                  <span className={`text-[13px] font-medium ${i === 0 ? 'text-white' : 'text-[#777]'}`}>{r.role}</span>
                  <span className="text-[11px] text-[#555] ml-auto shrink-0">{r.period}</span>
                </div>
              ))}
            </div>
          </div>
